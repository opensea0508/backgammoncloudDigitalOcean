...
export default class GameManager {
  ...
  applyMoveSequence(
    ...
    if (winResult?.isWin) {
      let winner = winResult.player;
      let loser = getOpponent(winner);
      if(winner == "W") {
        controller.updateWinTime(this.hostUsername);
        controller.updateLossTime(this.guestUsername);
      }
      else {
        controller.updateLossTime(this.hostUsername);
        controller.updateWinTime(this.guestUsername);
      }
